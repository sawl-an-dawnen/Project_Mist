...
public class Pickup : MonoBehaviour
{
    ...
    void PointArmAtObject()
    {
        ...
        // Apply rotation to the arm bone
        armBone01.rotation = Quaternion.Slerp(armBone01.rotation, Quaternion.Euler(0, 0, angle), Time.deltaTime * 10f);
        armBone02.rotation = Quaternion.Slerp(armBone02.rotation, Quaternion.Euler(0, 0, angle), Time.deltaTime * 10f);
        Debug.Log("Rotation Applied");
        Debug.DrawLine(armBone01.position, target.position, Color.red);  // First arm to target
        Debug.DrawLine(armBone02.position, target.position, Color.green); // Second arm to target
    }
